/**
 * search_eu_implementations — Search international instruments referenced by Ghanaian statutes.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_ghanaian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS ghanaian_statute_count,
  ...
  if (input.has_ghanaian_implementation !== undefined) {
    sql += input.has_ghanaian_implementation ? ` HAVING ghanaian_statute_count > 0` : ` HAVING ghanaian_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    ghanaian_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        ghanaian_statute_count: r.ghanaian_statute_count,
