/**
 * get_eu_basis — Retrieve international legal basis for a Ghanaian statute.
 * Covers EU regulations/directives, AU conventions, and other international instruments.
 */
...
export async function getEUBasis(
  ...
  const euDocuments: EUBasisDocument[] = rows.map(row => {
    const result: EUBasisDocument = {
      ...
      community: row.community, reference_type: row.reference_type,
