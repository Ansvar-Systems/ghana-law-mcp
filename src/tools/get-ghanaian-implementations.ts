/**
 * get_ghanaian_implementations — Find Ghanaian statutes implementing an
 * international directive, regulation, or convention.
 */

import type { Database } from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetGhanaianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface GetGhanaianImplementationsResult {
  eu_document_id: string;
  eu_title?: string;
...
export async function getGhanaianImplementations(
  db: Database,
  input: GetGhanaianImplementationsInput
): Promise<ToolResponse<GetGhanaianImplementationsResult>> {
  if (!input.eu_document_id) {
    throw new Error('eu_document_id is required');
  }

  // Get international document title
